...
export function AccountBalanceCheck({ address }: { address: PublicKey }) {
  ...
  if (query.isError || !query.data) {
    return (
      <AppAlert
        action={
          <Button variant="ghost" onClick={() => mutation.mutateAsync(1).catch((err) => console.log(err))}>
            Request Airdrop
          </Button>
        }
      >
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        <span className="font-mono">
                          <ExplorerLink label={(pubkey.toString())} path={`account/${pubkey.toString()}`} />
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
                ...
                {items?.map((item) => (
                  <TableRow key={item.signature}>
                    <TableHead className="font-mono">
                      <ExplorerLink path={`tx/${item.signature}`} label={(item.signature)} />
                    </TableHead>
